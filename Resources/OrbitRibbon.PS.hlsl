...
float4 main(PSInput input) : SV_TARGET
{
    ...
    const float edgeFade = 1.0f - smoothstep(0.42f, 1.0f, edge);
    const float headFade = smoothstep(0.0f, 0.12f, t);
    const float tailFade = 1.0f - smoothstep(0.78f, 1.0f, t);
    const float broken = lerp(0.58f, 1.0f, Hash(float2(floor(t * 18.0f), ribbonIndex + floor(gRibbonParams0.w * 2.0f))));
    const float frontBoost = lerp(0.34f, 1.0f, front);
    float alpha = input.color.a * edgeFade * headFade * tailFade * broken * frontBoost;
