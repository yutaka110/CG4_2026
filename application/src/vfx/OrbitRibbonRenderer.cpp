...
namespace {
struct OrbitRibbonDrawConstants {
    Matrix4x4 worldViewProjection{};
    Vector4 color{};
    Vector4 ribbonParams0{}; // orbitRadius, ribbonWidth, length, phase
    Vector4 ribbonParams1{}; // alpha, twist, turbulence, unused
};
...
bool IsOrbitRibbonComponent(const EffectComponentCommon& component) {
    return component.rendererId == "OrbitRibbonRenderer" || component.techniqueId == "OrbitRibbon";
}
} // namespace

void OrbitRibbonRenderer::RegisterPasses(
...
void OrbitRibbonRenderer::Draw(
    ...
    commandList->SetPipelineState(context.appPipelines->GetOrbitRibbonPSO());
    commandList->IASetVertexBuffers(0, 1, &context.scene->orbitRibbon.vbv);
    ...
    for (const CylinderRenderItem& item : queue) {
        if (item.common.instance == nullptr ||
            ...
            !IsOrbitRibbonComponent(*item.common.componentCommon)) {
            continue;
        }
        ...
        OrbitRibbonDrawConstants constants{};
        constants.worldViewProjection = Multiply(world, context.frameState->viewProjectionMatrix);
        constants.color = color;
        constants.ribbonParams0 = {
            (std::max)(0.01f, settings.topRadius),
            (std::max)(0.002f, settings.bottomRadius),
            (std::max)(0.05f, settings.height),
            context.beamTime * settings.uvScrollSpeed,
        };
        constants.ribbonParams1 = {
            alpha,
            5.65f,
            0.085f,
            0.0f,
        ...
        commandList->SetGraphicsRoot32BitConstants(
            0,
            static_cast<UINT>(sizeof(OrbitRibbonDrawConstants) / sizeof(uint32_t)),
            &constants,
            0);
        commandList->DrawInstanced(context.scene->orbitRibbon.vertexCount, 1, 0, 0);
    }
}
